...
contract BearCrushTOKENDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("BearCrushTOKEN_Dividend_Tracker", "BearCrushTOKEN_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "BearCrushTOKEN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "BearCrushTOKEN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BearCrushTOKEN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BearCrushTOKEN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "BearCrushTOKEN_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BearCrushTOKEN is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    BearCrushTOKENDividendTracker public dividendTracker;
...
% per tx:
2% to all holders in BNB
1% per tx to marketing wallet in BNB
1% per tx to developments wallet in BNB

Mktg
0x3Dd594f9cC628a16980aF7230708aBA92bD3df95

Devt
0x711d623cba424dAbA3B56eC55b88bDfB46062a69
...
Facilitate to upload code in github
Update code when audit needs some changes
Assist in mainnet deployment
*/
    constructor() ERC20("BearCrush TOKEN", "BCRUSH") {
        BNBRewardsFee = 2;
        liquidityFee = 0;
        marketingFee = 1;
        devFee = 1;
        ...
        onSellliquidityFee = 0;
        onSellmarketingFee = 1;
        onSellDevFee = 1;

        marketingWallet = payable(0x3Dd594f9cC628a16980aF7230708aBA92bD3df95); // owner
        developmentWallet = payable(0x711d623cba424dAbA3B56eC55b88bDfB46062a69); // 5 

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(devFee); // total fee transfer and buy

        onSelltotalFees = onSellBNBRewardsFee.add(onSellliquidityFee).add(onSellmarketingFee).add(onSellDevFee);

    	dividendTracker = new BearCrushTOKENDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BearCrushTOKEN: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BearCrushTOKEN: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BearCrushTOKEN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BearCrushTOKEN: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BearCrushTOKEN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BearCrushTOKEN: Cannot update gasForProcessing to same value");
